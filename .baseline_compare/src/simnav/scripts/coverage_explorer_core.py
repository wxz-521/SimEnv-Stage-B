...
def active_zone(portals, completed, split, band=1.5):
...
def target_switch_allowed(
    ...
    """Whether a fresher viewpoint may replace the one already being driven.

    Requirement (user): the *target point* may change before it is reached, not
    only its orientation.  The mechanism that used to do this was deleted
    because a pure score comparison made it oscillate -- the recorded symptom
    was "the target changed back and forth for an hour".  A switch is therefore
    allowed only when all of these hold:

    * the current target has been held for ``dwell_seconds`` (no ping-pong);
    * the candidate is room-owned, and belongs to the locked room when a room
      lock is held (never switch out from under a lock);
    * Pareto: the candidate's path is not longer than the current one (plus a
      small slack), so switching can never lengthen the trip;
    * the candidate is at least ``gain_ratio`` times better on the metric the
      target is actually worked for -- camera gain for a camera viewpoint,
      combined gain otherwise -- so near-equal neighbours do not trade places.

    A candidate that *is* the active one (or its identical twin) is never a
    switch, which also stops the same cell from being re-adopted repeatedly.
    """
    ...
    if (
        ...
        return False
    if float(candidate.path_length) > float(active.path_length) * path_slack + path_margin:
        return False
    metric = (
        "camera_gain"
        if candidate.kind in ("CAMERA_FRONTIER", "SPHERE_REVIEW")
        else "combined_gain"
    )
    current = float(getattr(active, metric, 0.0))
    fresh = float(getattr(candidate, metric, 0.0))
    if fresh <= 0.0:
        return False
    if current > 0.0 and fresh < current * float(gain_ratio):
        return False
    return True
...
class TaskCoveragePlanner:
    ...
    def plan(
        ...
        def corridor_wander_target():
            ...
            for reach in (2.0, -2.0, 4.0, -4.0, 6.0, -6.0, 9.0, -9.0, 14.0, -14.0):
        ...
        if topology_lock:
            ...
            # Far-zone straight transit (P3): while no room-interior frontier is
            # in sight yet, prefer the furthest-forward reachable corridor point
            # measured along the corridor axis, instead of chasing the highest
            # local laser gain.  run81's floor 1 spent 105 s of 190 s dispatching
            # small corridor frontiers; a forward leg needs only the driving
            # time.  Reachability is enforced by the path/clearance checks, and
            # any visible room frontier still wins (corridor = transit only).
            #
            ...
            # corridor direction, shortened until navigable).  That synthesises a
            # new target instead of ranking the frontier pool, so the corridor
            # contract tests must be updated in the same change.
            targets.sort(
                key=lambda item: (
...
# Portals closer to the entrance gate than this are the gate's OWN opening, not
# a room door.  The gate is by definition at along = 0, and the lobby/corridor
# transition has the same "wall + gap" signature as a doorway, so the detector
# kept registering it as rooms: on run104 `ROOM_L_0` (along 0.0 m) and
# `ROOM_R_2` (along 1.0 m) were held as verified door portals while the real
# front doors sit at along 7.5 m.  Those phantom rooms then demanded 0.55
# coverage and dragged the robot back to the entrance repeatedly.
GATE_PORTAL_EXCLUSION = 2.5
...
def detect_room_portals(
    ...
    """Room doorways, with the entrance gate's own opening excluded.

    A thin wrapper: detection itself is untouched, so the only behavioural
    change is that a portal within ``GATE_PORTAL_EXCLUSION`` metres of the gate
    is dropped.  Real front doors sit at along ~7.5 m, so nothing genuine is
    affected.
    """
    portals = detect_room_portals_impl(
        grid, gate_center, forward_yaw, forward_depth, lateral_half_width,
        corridor_half_width, minimum_along, portal_prefix, max_portal_width,
        minimum_jamb_support, passage_depth,
    )
    return tuple(
        portal for portal in portals
        if float(getattr(portal, "along", 0.0)) >= GATE_PORTAL_EXCLUSION
    )
