...
class CoverageExplorer:
    ...
    def _floor_context_callback(self, message):
        ...
        if floor_index <= 0:
            return
        # Upper floors are isomorphic to the ground floor: every floor detects
        # its own doorways from its own map.  The reference floor's geometry used
        # to be pre-seeded here (the floor_ready payload's "reused_topology"),
        # and on run80's top floor those portals kept evidence=1 while the live
        # map detected the real openings with evidence 61-72; every crossing
        # route aimed at the stale geometry failed (CANDIDATE_PATH_UNREACHABLE),
        # the floor produced only corridor fallback targets and never started.
        # Deleted in favour of live detection on every floor.
        reused_portals = ()
        with self.lock:
    ...
    def _control_impl(self, _event):
        ...
        if not complete and self.stuck_target_seconds > 0.0 and target is not None:
            idle = (rospy.Time.now() - self.last_progress_stamp).to_sec()
    ...
    def _switch_active_target(self, plan):
        ...
        self.active_path = tuple(candidate.path)
        self.path_index = 0
