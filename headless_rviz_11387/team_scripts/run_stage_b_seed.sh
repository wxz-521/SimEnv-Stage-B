...
if [ "${STAGE_B_ALLOW_EXISTING_PROCESSES:-0}" != "1" ]; then
  for process_pattern in \
    'gzserver' 'gzclient' 'fastlio_mapping' 'junior_ctrl' \
    'single_floor_explorer_node.py' 'danger_detector_node.py' \
    'stage_b_localization.launch' 'stage_b_behavior.launch' 'rviz -d' \
    'monitor_stage_b_actual.py'; do
...
echo "Stage B resource guard: ROS_PORT=$ROS_PORT CPU_LIST=${CPU_LIST:-inherited}" > "$RUN_DIR/resource_guard.log"
...
copy_artifacts() {
  cp -f "$WORKSPACE_DIR/generated_building/scene_manifest.json" "$RUN_DIR/" 2>/dev/null || true
  cp -f "$WORKSPACE_DIR/generated_building/layout_metadata.json" "$RUN_DIR/" 2>/dev/null || true
  cp -f "$WORKSPACE_DIR/results/danger_truth.json" "$RUN_DIR/" 2>/dev/null || true
  cp -f "$WORKSPACE_DIR/results/detected_danger.json" "$RUN_DIR/" 2>/dev/null || true
  cp -f "$WORKSPACE_DIR/results/detected_danger_debug.json" "$RUN_DIR/" 2>/dev/null || true
  cp -f "$WORKSPACE_DIR/logs/competition_gazebo.log" "$RUN_DIR/" 2>/dev/null || true
  cp -f "$WORKSPACE_DIR/logs/junior_ctrl.log" "$RUN_DIR/" 2>/dev/null || true
}

rm -f "$WORKSPACE_DIR/results/detected_danger.json" \
  "$WORKSPACE_DIR/results/detected_danger_debug.json" "$RUN_DIR/result.json"
...
"${RUN_PREFIX[@]}" setsid roslaunch simnav stage_b_localization.launch > "$RUN_DIR/navigation.log" 2>&1 &
NAV_PID=$!
...
if [ "$RUN_MODE" = "entry_only" ]; then
  # Subscribe before behavior starts so no entry transition is missed.
  "${RUN_PREFIX[@]}" python3 team_scripts/monitor_stage_b_entry.py \
    --sim-timeout "$SIM_TIMEOUT" --seed "$SEED_VALUE" \
    --output "$RUN_DIR/result.json" \
    > "$RUN_DIR/entry_monitor.log" 2>&1 &
  MONITOR_PID=$!
fi

"${RUN_PREFIX[@]}" setsid roslaunch simnav stage_b_behavior.launch \
  door_alignment_tolerance:="$DOOR_ALIGNMENT_TOLERANCE" \
  expected_rooms_per_floor:="$EXPECTED_ROOM_COUNT" \
  > "$RUN_DIR/behavior.log" 2>&1 &
BEHAVIOR_PID=$!
wait_for_topic /simnav/explorer_status 60

if [ "$RUN_MODE" != "entry_only" ]; then
  "${RUN_PREFIX[@]}" python3 team_scripts/monitor_stage_b_actual.py \
    --sim-timeout "$SIM_TIMEOUT" --seed "$SEED_VALUE" \
    --expected-rooms-per-floor "$EXPECTED_ROOM_COUNT" \
