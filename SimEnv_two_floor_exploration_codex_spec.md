...
>
> **公开基线**：代码已发布到 https://github.com/wxz-521/SimEnv-Stage-B ，默认分支 `main`，已验证提交为 `808e8b33455dbe25434b4607aa3b9ff26c033d26`。B-3 多 seed 验收必须从该提交或其明确记录的后继提交开始，测试中不得修改代码。
...
# 17. 2026-09-01 房间覆盖提速阶梯

当前房间综合覆盖改为相机主导的线性公式：

```text
combined = 0.05 * laser + 0.95 * camera
room_combined_coverage_target = 0.84
```

冻结 seed `20260902`、真实无界面 Gazebo、RViz 关闭的 0.01 粒度边界测试结果：

| 综合门槛 | 四房完成 | 红球结果 | 仿真时间 | 判断 |
| --- | --- | --- | --- | --- |
| 0.85 | 4/4 | 3/3，漏检 0 | 463.864 s | 通过 |
| 0.84 | 4/4 | 3/3，漏检 0 | 421.862 s | 通过，当前采用 |
| 0.83 | 4/4 | 2/3，漏检 1 | 359.942 s | 不通过 |

因此当前冻结 seed 上的最低无漏检门槛为 `0.84`，相对同一 5/95 公式下的
`0.85` 轮次节省约 `42.0 s` 仿真时间。该结论只建立单 seed、0.01 粒度的局部边界，
不替代 B-3 连续多 seed 稳定验收；在多 seed 证据完成前不得继续降到 `0.83`。

测试证据位于：

```text
logs/coverage_threshold_sweep_95cam_20260901/combined_085/seed_20260902/
logs/coverage_threshold_sweep_95cam_20260901/combined_084/seed_20260902/
logs/coverage_threshold_sweep_95cam_20260831/combined_083/seed_20260902/
```

同一批测试还多次出现前门无候选、房间 `BLOCKED` 和第四房 `NO_FRONTIER`，这些轮次已单独
保留为探索链异常，不计入覆盖门槛的红球边界。说明当前总耗时仍明显受门洞发现和返回路径
可执行时机影响，不能假设降低覆盖门槛会按比例缩短每个房间时间。

---

# 18. 2026-09-01 探索行走速度验证

在保持 5/95 综合覆盖公式和 `room_combined_coverage_target=0.84` 不变的前提下，仅提高
房间探索与走廊转场的通用 `motion_speed`。初始固定前进仍为 `0.45 m/s`；实际穿越门口时
继续执行 `min(motion_speed, 0.25)`，因此过门速度上限仍为 `0.25 m/s`。门检测、门宽、
导航 clearance、覆盖规划和危险物检测均未修改。

真实无界面 Gazebo、RViz 关闭的结果：

| seed | 通用速度 | 四房/退出 | 红球结果 | 阻塞/故障 | 仿真时间 |
| --- | --- | --- | --- | --- | --- |
| 20260902 | 0.45 m/s | 4/4，FLOOR_COMPLETE | 3/3，漏检 0 | 0 / 无 | 421.862 s |
| 20260902 | 0.55 m/s | 4/4，FLOOR_COMPLETE | 3/3，漏检 0 | 0 / 无 | 378.026 s |
| 20260902 | 0.60 m/s | 4/4，FLOOR_COMPLETE | 3/3，漏检 0 | 0 / 无 | 334.842 s |
| 20260903 | 0.60 m/s | 4/4，FLOOR_COMPLETE | 本层真值 0 | 0 / 无 | 389.916 s |

因此默认通用速度采用 `0.60 m/s`。同 seed 相对 0.45 m/s 节省 `87.020 s`（约 20.6%），
且两个 seed 均完成四房退出。此前独立 RL 速度扫测曾显示 0.60 m/s 附近存在实际速度中位数
偏低的停滞倾向，因此目前不继续提高到 0.75 m/s；后续 B-3 多 seed 验收仍需同时检查实际
位移、`navigation_blocks`、红球漏检和房间退出，不得只看 `/cmd_vel` 峰值。

测试证据位于：

```text
logs/coverage_speed_sweep_95cam_20260901/speed_055/seed_20260902/
logs/coverage_speed_sweep_95cam_20260901/speed_060/seed_20260902/
logs/coverage_speed_sweep_95cam_20260901/speed_060_verify/seed_20260903/
logs/rl_speed_sweep_20260825_v4/result.json
```
