...
class CoverageExplorer:
    def __init__(self):
        ...
        self.camera_weight = max(
            0.0, min(1.0, float(rospy.get_param("~camera_weight", 0.65)))
        )
        ...
        self.room_combined_coverage_target = max(
            0.0,
            min(1.0, float(rospy.get_param("~room_combined_coverage_target", 0.85))),
        )
        ...
        self.motion_speed = max(0.30, float(rospy.get_param("~motion_speed", 0.45)))
    ...
    def _plan(self, _event):
        with self.lock:
            if self.floor_complete or self.gate_source is None:
                return
    ...
    def _remember_front_portals(self, portals):
        ...
        for portal in portals:
            side_prefix = "ROOM_{}_".format(portal.side)
            existing = next(
                (
                    topology_id
                    for topology_id in self.front_station_topologies
                    if topology_id.startswith(side_prefix)
                ),
                None,
            )
    ...
    def _update_topology_lifecycle(self, plan, target_active):
        ...
        if state.get("state") == "RETURNING":
            ...
            if in_corridor:
                ...
                if lock in self.front_station_topologies and lock.startswith("ROOM_"):
                    self.completed_front_sides.add(lock.split("_")[1])
                self.topology_lock = None
                self.returning_topology = None
                self.active_target = None
                self.active_path = ()
    ...
    def _update_rear_transit(self, plan):
        ...
        self._remember_front_portals(plan.front_station_portals)
        remembered_sides = {
            topology_id.split("_")[1]
            for topology_id in self.front_station_topologies
            if topology_id.startswith("ROOM_")
        }
        front_complete = bool(
