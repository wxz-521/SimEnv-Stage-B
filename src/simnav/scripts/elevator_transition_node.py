...
class ElevatorTransition:
    ...
    def _navigation_map_callback(self, message):
        ...
        if gate is None or self.floor_complete:
            return
        expected_heading = normalize_angle(gate[2] - math.pi / 2.0)
        for candidate in detect_wide_lobby_openings(
            grid,
            gate[:2],
            gate[2],
            ...
            with self.lock:
                ...
                if evidence < self.elevator_candidate_confirm_cycles:
                    continue
                existing_width = (
                    float(self.elevator_portal[3])
                    if self.elevator_portal is not None
                    and len(self.elevator_portal) >= 4
                    else -float("inf")
                )
                if self.elevator_portal is None or candidate["width"] >= existing_width:
    ...
    def _drive_planned_to(
        ...
        if planar_distance(pose, target) <= self.target_tolerance:
            self._stop()
            return True
        now = rospy.Time.now()
        plan_expired = (
