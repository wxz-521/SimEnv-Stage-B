...
class CorridorExhaustionTest(unittest.TestCase):
    ...
    def test_corridor_exhaustion_does_not_deadlock_on_unfinished_rooms(self):
        ...
        # The rescued viewpoint is deliberately ownership-agnostic: portal ids
        # are transient bins of one physical doorway.  What must hold is the
        # anti-deadlock contract - a viewpoint is dispatched while an unfinished
        # doorway still exists - and, since the corridor became transit-only
        # (2026-09-13), that the dispatched viewpoint is owned by a ROOM rather
        # than by the corridor: touring the corridor while four doorways are
        # actionable is exactly the behaviour run79 showed (targets 14.05 ->
        # 3.28 m along the corridor, no room entered).  The corridor remains the
        # fallback when no room candidate exists at all.
        self.assertNotEqual(plan.target.topology_id, "CORRIDOR")
        self.assertTrue(
            plan.actionable_portals,
            "the corridor must still owe work while it dispatches this view",
        )
