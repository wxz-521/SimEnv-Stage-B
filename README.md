...
推荐使用仓库提供的 `docker/Dockerfile`。ROS1 Noetic 主机和 ROS2 主机都可以直接使用同一个
容器，源码通过卷挂载，编译产物默认落在仓库 `devel/`，不依赖某台机器私有的构建目录：
...
docker run --rm -it --net=host \
  ...
  simenv-noetic \
  bash -lc 'SIMENV_BUILD_JOBS=2 ./docker/build_workspace.sh'
```
...
如使用外部或缓存构建目录，设置 `SIMENV_DEVEL_DIR=/path/to/devel` 即可；启动脚本会优先使用
该目录，其次使用仓库 `devel/`，最后才回退到兼容旧环境的 `.simenv_build/devel`。
