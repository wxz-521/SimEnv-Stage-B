...
class TaskCoveragePlanner:
    ...
    def __init__(
        ...
        danger_sweep_enabled: bool = False,
        danger_sweep_clearance: float = 0.60,
        danger_sweep_spacing: float = 2.0,
        danger_sweep_min_remaining: int = 3,
    ):
        ...
        # Red-object sweep.  The red spheres are the scored danger sources and
        # the mission requires recall 1.0, so area coverage alone is not
        # enough: a sphere can sit in the one room corner the camera only ever
        # saw from far away.  ``danger_sweep_clearance`` matches the generator's
        # placement rule (0.45 m wall clearance + 0.15 m radius, which also
        # dominates the 0.35 m furniture clearance), and
        # ``danger_sweep_range`` matches the distance at which the fixed
        # camera actually resolved a sphere.
        self.danger_sweep_enabled = bool(danger_sweep_enabled)
        self.danger_sweep_clearance = max(0.0, float(danger_sweep_clearance))
        self.danger_sweep_spacing = max(0.4, float(danger_sweep_spacing))
        self.danger_sweep_min_remaining = max(1, int(danger_sweep_min_remaining))
    ...
    def plan(
        ...
        danger_covered: Optional[np.ndarray] = None,
    ) -> CoveragePlan:
        ...
        diagnostics = {
            ...
            "danger_sweep_feasible_cells": 0,
            "danger_sweep_remaining_cells": 0,
            "danger_sweep_targets": 0,
        ...
        # Red-object sweep (pure geometry, no colour model).  The generator
        # places every scored red sphere uniformly inside a room's feasible
        # interior: at least 0.45 m from the walls and 0.35 m from furniture.
        # Measured on run14 floor 0, the fixed forward camera resolved the
        # sphere whose closest approach was 2.30 m but missed the two whose
        # closest approach was 6.27 m and 6.83 m, and the whole trajectory had
        # been within 2.5 m of only 43% of that feasible zone.  Area coverage
        # is therefore not a recall guarantee.  While a room is locked, keep
        # dispatching stances until the robot's own path has passed within
        # ``danger_sweep_range`` of every reachable feasible cell, which is the
        # condition a detection actually needs.
        if (
            topology_lock
            and self.danger_sweep_enabled
            and danger_covered is not None
            and np.asarray(danger_covered).shape == data.shape
        ):
            feasible = room_region & reachable & (clearance >= self.danger_sweep_clearance)
            remaining = feasible & ~np.asarray(danger_covered, dtype=bool)
            remaining_count = int(np.count_nonzero(remaining))
            diagnostics["danger_sweep_feasible_cells"] = int(np.count_nonzero(feasible))
            diagnostics["danger_sweep_remaining_cells"] = remaining_count
            diagnostics["danger_sweep_targets"] = 0
            if remaining_count >= self.danger_sweep_min_remaining:
                stride = max(
                    1, int(math.ceil(self.danger_sweep_spacing / grid.resolution))
                )
                chosen_buckets = set()
                for row, column in zip(*np.nonzero(remaining)):
                    bucket = (int(row) // stride, int(column) // stride)
                    if bucket in chosen_buckets:
                        continue
                    chosen_buckets.add(bucket)
                    point = grid.cell_center(int(row), int(column))
                    cell = self._review_target(grid, reachable, distance, point)
                    if cell is None:
                        continue
                    path = self._path(grid, predecessor, seed, cell)
                    if not path:
                    ...
                    targets.append(
                        FrontierTarget(
                            ...
                            combined_gain=combined,
                            min_clearance=min(
                                clearance[grid.world_to_cell(*point_in_path)]
                                for point_in_path in path
                            ),
                            look_at=point,
                        )
                    )
                    diagnostics["danger_sweep_targets"] += 1
        ...
        if snapshot.camera < float(camera_target):
            ...
            if not all_camera_cells:
                # Every remaining unseen cell sits inside the revisit radius of
                # an already visited target, so the normal filter empties the
                # candidate pool.  The room then has no executable frontier
                # while its camera coverage is still low and the explorer waits
                # forever on a target that never arrives (observed: coverage
                # frozen at 0.453 with a repeated "no executable frontier"
                # warning for over 100 simulated seconds).  Fall back to the
                # nearest reachable viewpoints that still carry camera gain, so
                # the robot keeps sweeping the unseen interior instead of
                # stalling.  This is also the danger-directed behaviour: the
                # sweep is what puts a red sphere in front of the camera.
                relaxed = {}
                for row, column in zip(*np.nonzero(reachable & camera_unseen)):
                    cell = (int(row), int(column))
                    if distance[cell] < max(2, int(math.ceil(0.6 / grid.resolution))):
                        continue
                    bucket = (cell[0] // stride, cell[1] // stride)
                    previous = relaxed.get(bucket)
                    if previous is None or distance[cell] < distance[previous]:
                        relaxed[bucket] = cell
                all_camera_cells = sorted(
                    relaxed.values(), key=lambda item: distance[item]
                )
                diagnostics["camera_candidates_relaxed"] = len(all_camera_cells)
        ...
        targets = [
            replace(
                item,
                topology_id=(
                    # A sweep stance belongs to the room that is being swept.
                    # Deriving it from the stance would let a doorway-adjacent
                    # cell be labelled with a neighbouring portal, which the
                    # room-lock filter then drops and silently disables the
                    # sweep.
                    str(topology_lock)
                    if item.kind == "DANGER_SWEEP" and topology_lock
                    else topology_id_for_point(
        ...
        # A red-object review outranks everything, then the geometric sweep
        # that is what actually puts the camera close enough to see a sphere,
        # then ordinary coverage.
        priority = {
            "SPHERE_REVIEW": 0,
            "DANGER_SWEEP": 1,
            "CAMERA_FRONTIER": 2,
        ...
        if topology_lock:
            ...
            locked_camera = [
                ...
                if item.kind in ("CAMERA_FRONTIER", "SPHERE_REVIEW", "DANGER_SWEEP")
            ]
