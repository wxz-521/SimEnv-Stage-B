...
class CoverageExplorer:
    def __init__(self):
        ...
        self.planner = TaskCoveragePlanner(
            ...
            danger_sweep_enabled=bool(
                rospy.get_param("~danger_sweep_enabled", False)
            ),
            danger_sweep_clearance=float(
                rospy.get_param("~danger_sweep_clearance", 0.60)
            ),
            danger_sweep_spacing=float(
                rospy.get_param("~danger_sweep_spacing", 2.0)
            ),
        )
        ...
        self.camera_points_memory_limit = max(
            1000, int(rospy.get_param("~camera_points_memory_limit", 400000))
        )
        ...
        self.door_search_enabled = bool(
            rospy.get_param("~door_search_enabled", True)
        )
        ...
        # camera coverage is still low.  Bounds the "no executable frontier"
        # stall seen on the second floor (coverage frozen for >100 s).
        self.room_frontier_wait_cycles = max(
            5, int(rospy.get_param("~room_frontier_wait_cycles", 20))
        )
        ...
        # Geometric red-object sweep.  The scored spheres sit in a room's
        # feasible interior and the mission needs recall 1.0, but a sphere is
        # only resolvable from close range: on run14 floor 0 the detected
        # sphere's closest approach was 2.30 m while the two missed ones were
        # never nearer than 6.27 m / 6.83 m, and only 43% of the feasible zone
        # had ever been within 2.5 m of the path.  This mask records the cells
        # the robot's own trajectory has already passed close to, which is the
        # condition a detection actually requires.  It is pure geometry and
        # uses no colour information and no learned model.
        self.danger_sweep_enabled = bool(
            rospy.get_param("~danger_sweep_enabled", False)
        )
        self.danger_sweep_range = max(0.5, float(rospy.get_param("~danger_sweep_range", 2.5)))
        self.danger_sweep_clearance = max(
            0.0, float(rospy.get_param("~danger_sweep_clearance", 0.60))
        )
        self.danger_sweep_spacing = max(
            0.4, float(rospy.get_param("~danger_sweep_spacing", 2.0))
        )
        self.danger_covered = None
    ...
    def _floor_context_callback(self, message):
        ...
        with self.lock:
            ...
            # The sweep mask is a per-floor trajectory record: an upper floor
            # reuses the same 2-D SLAM frame, so carrying it over would make
            # the sweep believe the new floor was already covered.
            self.danger_covered = None
    ...
    def _mark_danger_covered(self, grid, pose):
        """Record the cells the trajectory has passed close enough to resolve.

        ``danger_sweep_range`` is the radius within which the fixed forward
        camera actually resolved a 0.15 m red sphere, so the union of these
        disks is exactly the set of feasible sphere placements this run could
        have detected.  Kept per floor and monotonic; the planner keeps
        dispatching stances until the set covers the room's feasible interior.
        """
        if not self.danger_sweep_enabled or grid is None or pose is None:
            return
        data = np.asarray(grid.data)
        if self.danger_covered is None or self.danger_covered.shape != data.shape:
            self.danger_covered = np.zeros(data.shape, dtype=bool)
        resolution = float(grid.resolution)
        radius_cells = int(math.ceil(self.danger_sweep_range / resolution))
        center = grid.world_to_cell(float(pose[0]), float(pose[1]))
        row, column = int(center[0]), int(center[1])
        row_start = max(0, row - radius_cells)
        row_stop = min(data.shape[0], row + radius_cells + 1)
        column_start = max(0, column - radius_cells)
        column_stop = min(data.shape[1], column + radius_cells + 1)
        if row_start >= row_stop or column_start >= column_stop:
            return
        rows = (
            np.arange(row_start, row_stop, dtype=np.float64) - float(row)
        ) * resolution
        columns = (
            np.arange(column_start, column_stop, dtype=np.float64) - float(column)
        ) * resolution
        disk = rows[:, None] ** 2 + columns[None, :] ** 2 <= self.danger_sweep_range ** 2
        self.danger_covered[row_start:row_stop, column_start:column_stop] |= disk
    ...
    def _plan(self, _event):
        ...
        with self.lock:
            ...
            reviewed = set(self.reviewed_hypotheses)
        self._mark_danger_covered(grid, pose)
        plan = self.planner.plan(
            ...
            danger_covered=self.danger_covered,
        )
        with self.lock:
            seen_portal_ids = set()
    ...
    def _update_topology_lifecycle(self, plan, target_active):
        ...
        state["state"] = "EXPLORING"
        self.topology_miss_cycles[lock] = 0
        # The lock may be held for a while in case a map update exposes a new
        # frontier, but the wait must be bounded.  When the relaxed camera sweep
        # also finds nothing the room simply cannot be completed from here and
        # the floor would otherwise stall until the run timeout.
    ...
    def _publish_status(self):
        with self.lock:
            ...
            payload = {
                ...
                "danger_sweep_enabled": self.danger_sweep_enabled,
                "danger_sweep_range": self.danger_sweep_range,
                "danger_sweep_covered_cells": (
                    int(np.count_nonzero(self.danger_covered))
                    if self.danger_covered is not None
                    else 0
                ),
                "danger_sweep_feasible_cells": int(
                    (extent.diagnostics or {}).get(
                        "danger_sweep_feasible_cells", 0
                    )
                    if extent is not None
                    else 0
                ),
                "danger_sweep_remaining_cells": int(
                    (extent.diagnostics or {}).get(
                ...
                "danger_sweep_targets": int(
                    (extent.diagnostics or {}).get("danger_sweep_targets", 0)
                    if extent is not None
                    else 0
                ),
