...
class TaskCoveragePlanner:
    ...
    def plan(
        ...
        if topology_lock:
            # Portal ids are 0.5 m longitudinal bins of a single physical
            # doorway and they drift as SLAM refines the wall, so a target for
            # the locked room can be labelled with a neighbouring bin instead
            # of the locked id.  Matching the id exactly then emptied the pool
            # and the node burned its whole station budget without ever
            # entering the room: on run26 floor 0 the explorer locked
            # ROOM_R_15, reported candidate_topologies == () with
            # last_plan_reason NO_FRONTIER for 59 consecutive sim seconds, and
            # released the lock without entering, while the same doorway was
            # simultaneously known as ROOM_R_30/36/39/43/48/52/56.  Accept any
            # bin of the same physical door, using the same tolerance the
            # completion test already applies.
