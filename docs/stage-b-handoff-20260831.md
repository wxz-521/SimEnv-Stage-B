...
当前版本已完成 Stage B 的 B-1 单 seed 功能闭环，但还不能宣称整个 Stage B 稳定验收完成；B-3 连续 5 seed 验收仍待执行。
...
3. 旧轮次未完成四房；当前正验收已完成 4/4 并发布 `floor_complete=true`。
4. 旧轮次危险物未通过；当前正验收为 3/3、漏检 0、误报 0。
5. 旧轮次未有效验证后方房间；当前正验收已完成左右后房探索和退出。

当前准确状态应写成：

> B-1 四房功能闭环和危险物零漏检已在一个冻结 seed 上通过；门洞缓存、进门路径与 RETURNING 隔离根因已修复。下一步冻结公开基线提交执行至少连续 5 seed，不再调整门检测、房内评分、覆盖阈值或走廊调度，除非出现可复现且证据明确的新故障。
...
| 房内使用局部信息增益前沿 | 当前生成 CAMERA_FRONTIER，并按 camera_weight=0.95 选择；仍依赖全局地图可达性、任务 mask 和 topology_id_for_point | 部分实现 | 可能出现门已确认但没有可用门内前沿 |
...
| camera_weight | 0.95 |
| motion_speed | 0.60 m/s（房间/走廊通用速度；实际过门仍由代码限为 0.25 m/s） |
...
7. **历史上曾出现覆盖率口径混用。** 旧版本房间完成使用 0.85 的线性 10/90，同时保留 `combined_coverage_target=0.84` 等旧全局参数和 harmonic 注释。2026-09-01 已将实际房间公式统一为 0.84 的线性 5/95；旧全局字段仍只作诊断，不决定房间完成。
...
9. **2026-09-01 将通用探索速度由 0.45 提升到 0.60 m/s。** 只调整房内探索和走廊转场的通用速度；初始固定前进仍是 0.45 m/s，实际过门仍由 `min(motion_speed, 0.25)` 限为 0.25 m/s，未修改门检测、门宽、避障间距或覆盖逻辑。冻结 seed `20260902` 在真实无界面 Gazebo 中由 421.862 s 降至 334.842 s，四房、红球 3/3、退出均通过；复核 seed `20260903` 用时 389.916 s，四房退出通过且该层真值红球为 0。
...
> 2026-09-05 追加冻结：在原有 1 m 停走复扫中加入相对走廊中心线的微小反偏转向，
> 不增加原地偏头或额外前进阶段，合成方向修正仍限制为 0.12 rad。真实无界面 Gazebo、
> RViz 关闭条件下，seed 20260901--20260905 门候选短测 5/5 在 44.914--48.910 s
> 达到 3/3 确认；后三个 seed 均在约 1.02 m 分段试探后首次获得候选。当前冻结校验为
> `config/stage_b_floor0_frozen.sha256`，后续多层工作只允许修改楼层/电梯衔接外围，
> 不再修改该清单覆盖的三个文件。
